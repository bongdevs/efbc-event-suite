...
class EFBC_Admin {
        ...
        <div class="wrap">
            ...
            <form method="post" action="options.php">
                ...
                <table class="form-table">
                    ...
                    <tr valign="top">
                        <th scope="row"><?php esc_html_e('Table Row Text Color', 'efbc-event-suite'); ?></th>
                        <td>
                            <input type="color" name="efbc_table_row_text" value="<?php echo esc_attr(get_option('efbc_table_row_text', '#333333')); ?>" />
                            <p class="description"><?php esc_html_e('Text color for table data rows.', 'efbc-event-suite'); ?></p>
                        </td>
                    </tr>
                    <tr valign="top">
                    ...
                    <tr valign="top">
                        <th scope="row"><?php esc_html_e('Odd Row Background', 'efbc-event-suite'); ?></th>
                        <td>
                            <input type="color" name="efbc_table_odd_row_bg" value="<?php echo esc_attr(get_option('efbc_table_odd_row_bg', '#ffffff')); ?>" />
                            <p class="description"><?php esc_html_e('Background color for odd table rows.', 'efbc-event-suite'); ?></p>
                        </td>
                    </tr>
                    <tr valign="top">
                    ...
                    <tr valign="top">
                        <th scope="row"><?php esc_html_e('Pagination Primary Color', 'efbc-event-suite'); ?></th>
                        <td>
                            <input type="color" name="efbc_pagination_primary" value="<?php echo esc_attr(get_option('efbc_pagination_primary', '#0073aa')); ?>" />
                            <p class="description"><?php esc_html_e('Base color for pagination buttons.', 'efbc-event-suite'); ?></p>
                        </td>
                    </tr>
                    <tr valign="top">
                        <th scope="row"><?php esc_html_e('Pagination Hover Color', 'efbc-event-suite'); ?></th>
                        <td>
                            <input type="color" name="efbc_pagination_hover" value="<?php echo esc_attr(get_option('efbc_pagination_hover', '#005a87')); ?>" />
                            <p class="description"><?php esc_html_e('Color for pagination buttons on hover.', 'efbc-event-suite'); ?></p>
                        </td>
                    </tr>
                    <tr valign="top">
                        <th scope="row"><?php esc_html_e('Pagination Active Color', 'efbc-event-suite'); ?></th>
                        <td>
                            <input type="color" name="efbc_pagination_active" value="<?php echo esc_attr(get_option('efbc_pagination_active', '#006799')); ?>" />
                            <p class="description"><?php esc_html_e('Color for active/current page button.', 'efbc-event-suite'); ?></p>
                        </td>
                    </tr>
                    <tr valign="top">
                        <th scope="row"><?php esc_html_e('Pagination Text Color', 'efbc-event-suite'); ?></th>
                        <td>
                            <input type="color" name="efbc_pagination_text" value="<?php echo esc_attr(get_option('efbc_pagination_text', '#ffffff')); ?>" />
                            <p class="description"><?php esc_html_e('Text color for pagination buttons.', 'efbc-event-suite'); ?></p>
                        </td>
                    </tr>
                    <tr valign="top">
                        <th scope="row"><?php esc_html_e('Table Font Family', 'efbc-event-suite'); ?></th>
                        <td>
                            <select name="efbc_table_font_family">
                                <option value="inherit" <?php selected(get_option('efbc_table_font_family', 'inherit'), 'inherit'); ?>>Use Theme Font (Inherit)</option>
                                <option value="-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen-Sans, Ubuntu, Cantarell, 'Helvetica Neue', sans-serif" <?php selected(get_option('efbc_table_font_family'), '-apple-system, BlinkMacSystemFont, \'Segoe UI\', Roboto, Oxygen-Sans, Ubuntu, Cantarell, \'Helvetica Neue\', sans-serif'); ?>>System Font Stack</option>
                                <option value="Georgia, 'Times New Roman', serif" <?php selected(get_option('efbc_table_font_family'), 'Georgia, \'Times New Roman\', serif'); ?>>Georgia Serif</option>
                                <option value="'Courier New', monospace" <?php selected(get_option('efbc_table_font_family'), '\'Courier New\', monospace'); ?>>Courier Monospace</option>
                            </select>
                            <p class="description"><?php esc_html_e('Font family for the attendee tables. "Use Theme Font" will inherit from your WordPress theme.', 'efbc-event-suite'); ?></p>
                        </td>
                    </tr>
                </table>
                <?php submit_button(); ?>
    ...
    public function enqueue_frontend_assets() {
        ...
        wp_enqueue_script('efbc-attendees-js', EFBC_ES_PLUGIN_URL . 'assets/js/front.js', ['jquery'], EFBC_ES_VERSION, true);

        // Localize ajaxUrl for frontend pagination script
        ...
        $header_bg = get_option('efbc_table_header_bg', '#0073aa');
        $header_text = get_option('efbc_table_header_text', '#ffffff');
        $row_text = get_option('efbc_table_row_text', '#333333');
        $row_hover = get_option('efbc_table_row_hover', '#f1faff');
        $odd_row = get_option('efbc_table_odd_row_bg', '#ffffff');
        $even_row = get_option('efbc_table_even_row_bg', '#f9f9f9');
        $pagination_primary = get_option('efbc_pagination_primary', '#0073aa');
        $pagination_hover = get_option('efbc_pagination_hover', '#005a87');
        $pagination_active = get_option('efbc_pagination_active', '#006799');
        $pagination_text = get_option('efbc_pagination_text', '#ffffff');
        $font_family = get_option('efbc_table_font_family', 'inherit');

        $inline_css = "
        :root {
            --efbc-header-bg: {$header_bg} !important;
            --efbc-header-text: {$header_text} !important;
            --efbc-text: {$row_text} !important;
            --efbc-row-hover: {$row_hover} !important;
            --efbc-odd-row: {$odd_row} !important;
            --efbc-even-row: {$even_row} !important;
            --efbc-primary: {$pagination_primary} !important;
            --efbc-primary-hover: {$pagination_hover} !important;
            --efbc-active: {$pagination_active} !important;
            --efbc-btn-text: {$pagination_text} !important;
        }
        .efbc-attendees-table thead tr{ background-color: {$header_bg} !important; color: {$header_text} !important; }
        .efbc-attendees-table thead th{ background-color: {$header_bg} !important; color: {$header_text} !important; }
        .efbc-attendees-table tbody td{ color: {$row_text} !important; }
        .efbc-attendees-table tbody tr:hover{ background-color: {$row_hover} !important; }
        .efbc-attendees-table tbody tr:nth-child(even){ background-color: {$even_row} !important; }
        .efbc-attendees-table, .efbc-attendees-table-wrapper { font-family: {$font_family} !important; }
        ";
