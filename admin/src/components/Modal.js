...
const Modal = ({ open, title, loading, data, onClose, eventId, activity, page = 1, lastPage = 1, total = 0, onPageChange = () => {}, onRefresh = () => {} }) => {
    ...
    useEffect(() => {
        ...
        fetch(`${efbcData.ajaxUrl}?action=efbc_get_saved_columns&nonce=${efbcData.nonce}`)
            .then(res => res.json())
            .then(res => {
                if (res.success && res.data[eventId]) {
                    ...
                    if (savedCols) {
                        setColumns(savedCols.map(item => typeof item === 'string' ? item : (item?.name ?? String(item))));
                    }
                }
            })
                ...
                {loading ? <p>Loading...</p> : data.length > 0 ? (
                    <div className="efbc-admin-table-wrapper">
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                            <div>Showing <strong>{data.length}</strong> of <strong>{total}</strong> attendees</div>
                        ...
                        <div style={{ marginTop: '12px', display: 'flex', justifyContent: 'center', gap: '6px', alignItems: 'center' }}>
                            ...
                            {[...Array(lastPage)].map((_, i) => {
                                ...
                                return (
                                    <button
                                        key={p}
                                        className={`button ${p === currentPage ? 'button-primary' : ''}`}
                                        onClick={() => onPageChange(p)}
                                    >{p}</button>
                                );
                            })}

                            <button className="button" disabled={currentPage >= lastPage} onClick={() => onPageChange(currentPage + 1)}>Next</button>
                        </div>
                    </div>
